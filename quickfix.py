...
while((num % 2) == 0):
	...
	if num > 0:
		time.sleep(1)
		print('\n\nThis the sentence you currently have:')
		time.sleep(1.5)
		print(f'\n\"{userSTR}\"')
		time.sleep(3)
		rep = input('\n\nWould you like to use something new?\n\nYes or No ——>')
	...
	if choice.strip().lower() == 'a':
		...
		if formal.strip().lower() == 'yes' or formal.strip().lower() == 'y' or formal.strip().lower() == 'ye' or formal.strip().lower() == 'ya' or formal.strip().lower() == 'e' or formal.strip().lower() == 'es':
			...
			userSTR = newtext.replace('. ', ' . ').replace('! ', ' ! ').replace('? ', ' ? ')
			
			spell = SpellChecker()
			tocheck = userSTR.split()
			misspelled = spell.unknown(tocheck)
			for x in misspelled:
				for i, check in enumerate(tocheck):
					if check == x:
						tocheck[i] = spell.correction(x)
			...
			i3 = i2.replace(' i!', ' I! ')
			i4 = i3.replace(' i?', ' I? ')
			i5 = i4.replace(' im ', ' I am ')
			i7 = i5.replace(" id ", " I would ")
			i8 = i7.replace(" ive ", " I have ")
			i9 = i8.replace(" i'm ", " I am ")
			i10 = i9.replace(" i'll ", " I will ")
			i11 = i10.replace(" i'd ", " I would ")
			output2 = i11.replace(" i've ", " I have ")
	...
	#custom
		...
		#expand contraction
			...
			i10 = i9.replace("i'll", "I will")
			i11 = i10.replace("i'd", "I would")
			output6 = i11.replace("i've", "I have")
...
if feedback.strip().lower() == 'yes' or feedback.strip().lower() == 'y' or feedback.strip().lower() == 'ye' or feedback.strip().lower() == 'ya':
	...
	response = input('-->')
	time.sleep(1)
	print('\nAnd lastly, what is your name? (for the feedback form)')
	time.sleep(1)
	username = input('-->')
	
	today = str(date.today())
	file = open('Feedback-Response.txt', 'a')
	file.write(response + '\n    --' + username + today + '\n')
	file.close()
	...
	msg['From'] = fromaddr
	msg['To'] = toaddr
	msg['Subject'] = f"Feedback on {name}"
	body = getpass.getuser()
	msg.attach(MIMEText(body, 'plain'))
	path = os.getcwd()
	editPath = path.replace("C:\\Users\\", "")
	filename = "Feedback-Response.txt"
	attachment = open(path + "\\Feedback-Response.txt", "rb")
	...
	time.sleep(1)
	print('Would you like to keep a copy of your response?\n')
	time.sleep(1)
	receipt = input('Yes or No\n-->')
	if feedback.strip().lower() == 'yes' or feedback.strip().lower() == 'y' or feedback.strip().lower() == 'ye' or feedback.strip().lower() == 'ya':
		time.sleep(1)
		print(f'You will find the file in the file directory that {name} is in.\n')

	else:
		os.remove("Feedback-Response.txt")
